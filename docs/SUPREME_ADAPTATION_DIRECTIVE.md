...
- **画布事件防拦截机制** - 触摸事件正常传递
- **精确间距控制** - 移动端界面紧凑优化
- **横屏极限空间利用** - windowHeight - 24 算法最大化画布
- **纯净渲染机制** - 移除投影与边框，消除Webkit亚像素黑线

#### 🔄 **iPad端核心机制**
- **智能设备检测切换** - 横竖屏布局自动选择
- **静态背景性能优化** - 避免动态背景卡顿
- **浏览器兼容性保护** - 微信等特殊环境适配
- **dvh 视口基准控制** - 强制使用 `100dvh` 解决浏览器地址栏偏移
- **PWA 沉浸式支持** - 必须维持 `manifest.json` 确保 Standalone 体验
...
// ❌ 禁止删除：dvh 视口保护
style={{
  minHeight: '100dvh', // 🚨 不可修改为 100vh：防止移动浏览器偏移
}}

// ❌ 禁止修改：全屏模式交互保护
// 🚨 严禁在全屏拦截器中阻止默认点击行为 (preventDefault on touchstart/touchend)
```
...
10. **单图响应式背景方案** - 跨端资源优化
11. **dvh 视口基准单位** - 移动端垂直居中基石
12. **全屏沉浸渲染策略** - 隐藏版权信息及放开点击拦截
