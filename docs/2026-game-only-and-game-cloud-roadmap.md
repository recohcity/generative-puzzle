...
- `main`：保留当前完整工程（历史参考与治理资产基线）。
  - 当前状态：历史上可直接发布到 GitHub Pages。
  - 过渡策略：切换到新发布链路后，`main` 不再作为纯游戏交付发布源。
- `backup-main-YYYYMMDD`：一次性快照备份，防误操作与兜底回退。
- `game-only`：纯游戏交付开发分支，专注前端单机可玩与轻量发布。
- `gh-pages`：仅存放静态构建产物，用于 GitHub Pages 线上站点。
  - 发布来源：由 `game-only` 自动构建并发布（`game-only -> build -> gh-pages`）。
- `game-cloud`：前后端重构版，面向账户体系与跨端数据一致（下一阶段主线）。
...
### 阶段 2：GitHub Pages 过渡发布（已确定采用推荐方案）

**目标**：将现有“main 直接发布”平滑切换到 `game-only -> build -> gh-pages`。

#### 发布链路

1. 在 `game-only` 提交变更。
2. CI 执行静态构建（必须启用 `BUILD_STATIC=true`）。
3. 将构建产物发布到 `gh-pages` 分支。
4. GitHub Pages Source 指向 `gh-pages`。
5. 验证线上稳定后，确认 `main` 退出纯游戏交付发布链路。
