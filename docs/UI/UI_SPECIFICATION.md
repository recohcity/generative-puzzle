# 🧩 Generative Puzzle 核心 UI/UX 规范手册 (V4.2 - 2026 极致全端适配版)
...
*   **数值呈现规范**:
    ...
    *   **标准数字 (New)**: 2026-04-28 强制。**必须使用 `font-sans`**。严禁在面向用户的分数、倍率、计时等数值中使用带有中斜线 0 的 `font-mono`。
...
*   **状态辨识**: 
    ...
    *   **状态防御**: 必须挂载 `.text-zoom-lock` 样式。

### 3.4 全平台结算系统规范 (Settlement Dashboard System)
*   **设计哲学**: 采用“仪表盘化”排版架构，将胜利反馈从“账单展示”进化为“多维成就墙”。
*   **核心分数字号**: **全端统一（Desktop/Mobile Portrait/Landscape）锁定为 `text-[56px] font-medium`**，颜色锁定 `brand-peach`。
*   **数值无缝化**: 严禁在任何分数显示中使用千分位分隔符（如 `1234` 而非 `1,234`），且统一使用 `font-sans`（无斜线 0）。
*   **三区架构 (Zone-based Layout)**:
    - **Zone 1: 品牌与荣誉**: 奖杯与难度标签。
    - **Zone 2: 核心成就**: 巨幕分数、星级、勋章墙。
    - **Zone 3: 深度解析**: 计分明细列表、操作按钮。
*   **弹窗标准**: 横屏下最大宽度提升至 `620px`，确保三区横向展开时具备呼吸感。
...
---
*版本：V4.2 (Detailed Technical Manual)*
*最近同步：2026-04-28 (移动端结算大一统与标准数字审美版)*
