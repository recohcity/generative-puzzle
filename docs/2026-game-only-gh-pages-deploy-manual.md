...
确保 `next.config.mjs`（或 `next.config.js`）具备静态导出配置（示例）：
...
> - `images.unoptimized: true`：避免依赖 Next Image 服务端优化。
> - `trailingSlash: true`：通常对 Pages 目录索引更友好。
