# 项目结构（项目地图）
> 修订日期：2025-01-24 (v1.3.36 桌面端画布居中修复完成更新)
...
- `configuration/`：配置文档目录（模块化拆分，v1.3.36更新）
  - `README.md`：配置文档索引，快速导航和使用指南
  - `01-core-architecture.md`：核心架构配置
  - `02-unified-managers.md`：统一架构管理器配置 (v1.3.33+)
  - `03-mobile-adaptation.md`：移动端适配配置 (v1.3.34+)
  - `04-unified-adaptation.md`：统一适配系统配置 (v1.3.35+)
  - `05-desktop-centering.md`：桌面端画布居中配置 (v1.3.36+)
  - `06-difficulty-cutting.md`：难度与切割配置
  - `11-device-responsive.md`：设备适配与响应式配置
  - `19-debug-mode.md`：F10 Debug 调试模式配置
  - 其他配置文档：游戏核心、媒体视觉、开发测试等模块（待完善）
...
- **智能数据源选择**：根据适配阶段选择正确的数据源（baseShape vs originalShape）
- **同步适配系统**：目标形状、拼图块、提示区域完美同步适配
- **桌面端体验完善**：窗口调整时画布重新居中，游戏功能完全恢复
- **跨平台兼容保证**：修复不影响移动端，保持系统整体一致性
- **修复评级A+**：桌面端画布居中达到卓越级别，彻底解决核心用户体验问题
...
- **v1.3.36重要里程碑**：桌面端画布居中修复完成，创新手动适配机制，彻底解决核心用户体验问题
...
- **桌面端修复成果**：无限循环彻底解决，手动适配机制创新，累积错误修复，窗口调整完美居中