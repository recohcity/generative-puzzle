...
|--------|------|------|
| 总目录数 | 60 | 项目目录结构层次 |
| 总文件数 | 292 | 代码文件和资源文件总数 |
| 项目容量 | 5.98 MB | 不包含node_modules的项目大小 |
...
📁 **e2e/**
  📁 **temp/**
    📄 `completed-puzzle-resize-fix.spec.ts`
    📄 `puzzle_state_adaptation_test_fixed.spec.ts`
    📄 `step3_puzzle_pieces_adaptation_test.spec.ts`
...
📁 **temp/**
  ...
  📁 **src/**
    📁 **config/**
      📄 `deviceConfig.js`
  📄 `test-device-detection-comprehensive.js`
  📄 `test-functionality-preservation.js`
  📄 `test-mobile-adaptation-verification.js`
📁 **test-results/**
📁 **tests/**
  📁 **canvas-adaptation/**
    📄 `test-canvas-adaptation.html`
    📄 `test-desktop-adaptation.html`
    📄 `test-iphone16pro-adaptation.html`
    📄 `test-ultrawide-desktop.html`
  📁 **shape-adaptation/**
    📄 `test-shape-adaptation.html`
  📄 `completed-puzzle-resize-debug.html`
  📄 `completed-puzzle-resize-fix-test.html`
  📄 `test-adaptation-engine-refactoring.ts`
  📄 `test-code-quality-assessment.js`
  📄 `test-component-functionality.js`
  📄 `test-comprehensive-functionality.ts`
  📄 `test-cross-device-compatibility.js`
  📄 `test-device-detection-comprehensive.ts`
  📄 `test-device-detection-unified.js`
  📄 `test-device-detection-unified.ts`
  📄 `test-device-manager-refactoring.ts`
  📄 `test-error-handling.ts`
  📄 `test-event-response-optimization.ts`
  📄 `test-functionality-preservation.ts`
  📄 `test-improved-iphone16-detection.ts`
  📄 `test-mobile-adaptation-verification.ts`
  ...
  📄 `test-resize-observer-replacement.ts`
  📄 `test-responsibility-separation.ts`
  📄 `test-setTimeout-removal.ts`
  📄 `test-system-validation.js`
  📄 `test-useCanvas-refactoring.ts`
  📄 `validate-responsibility-separation.js`
📁 **types/**
  📄 `common.ts`
  📄 `global.d.ts`
  📄 `memory.ts`
...
*📅 生成时间：2025/7/28 23:48:45 | 🔧 版本：v3.1.0 | 🎯 开发导航工具*
