...
> 2. **启蒙难度优化**：难度 Level 1 的碎片范围从 3-4 片优化为 **2-4 片**，提升了入门关卡的随机性。
...
|---------|------------|------------|---------|---------|---------|
| 1       | 2条        | 2-4片      | 启蒙     | 500     | 1.0     |
| 2       | 3条        | 4-6片      | 寻迹     | 800     | 1.2     |
| 3       | 4条        | 5-8片      | 灵动     | 1200    | 1.5     |
| 4       | 6条        | 7-12片     | 巧思     | 1800    | 1.8     |
| 5       | 8条        | 9-16片     | 破界     | 2500    | 2.2     |
| 6       | 10条       | 11-20片    | 幻影     | 3500    | 2.8     |
| 7       | 12条       | 13-24片    | 渊薮     | 5000    | 3.5     |
| 8       | 15条       | 16-30片    | 神工     | 8000    | 5.0     |
...
const DIFFICULTY_SETTINGS = {
  1: { targetCuts: 2,  pieceRange: { min: 2, max: 4 },   baseScore: 500,  label: '启蒙' },
  2: { targetCuts: 3,  pieceRange: { min: 4, max: 6 },   baseScore: 800,  label: '寻迹' },
  3: { targetCuts: 4,  pieceRange: { min: 5, max: 8 },   baseScore: 1200, label: '灵动' },
  4: { targetCuts: 6,  pieceRange: { min: 7, max: 12 },  baseScore: 1800, label: '巧思' },
  5: { targetCuts: 8,  pieceRange: { min: 9, max: 16 },  baseScore: 2500, label: '破界' },
  6: { targetCuts: 10, pieceRange: { min: 11, max: 20 }, baseScore: 3500, label: '幻影' },
  7: { targetCuts: 12, pieceRange: { min: 13, max: 24 }, baseScore: 5000, label: '渊薮' },
  8: { targetCuts: 15, pieceRange: { min: 16, max: 30 }, baseScore: 8000, label: '神工' }
}
```
...
- 每片平均时间：5秒（优化后）
- 基础时间：4 × 5 = 20秒
- 光速（少于0.8倍）：少于16秒内 → +1000分（1000 × 1.0，极限挑战）
...
- 每片平均时间：18秒（优化后）
- 基础时间：30 × 18 = 540秒（9分钟）
- 光速（少于0.8倍）：少于432秒（7分12秒）内 → +2000分（1000 × 2.0）
...
- 光速（少于0.8倍）：少于128秒（2分8秒）内 → +1500分（1000 × 1.5）
...
**挑战性提示**：
- 每个难度都有7个速度等级（光速/极速/快速/良好/标准/一般/慢）
- 前6个等级有速度奖励，第7个等级（慢）无奖励，鼓励玩家重新挑战
- 玩家可以挑战更高等级的速度奖励，获得更高分数
- 高难度虽然时间阈值更长，但奖励分数也更高，鼓励玩家挑战极限

**示例挑战目标**（难度8，30片拼图）：
- 💫 **光速**：少于432秒（7分12秒）内 → +2000分（1000 × 2.0）
- 🏆 **极速**：少于540秒（9分钟）内 → +1200分（600 × 2.0）
