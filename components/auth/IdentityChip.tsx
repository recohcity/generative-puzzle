...
export default function IdentityChip({ onClick, onClose, isPanelOpen = false, className, panelScale = 1 }: IdentityChipProps) {
  ...
  return (
    <div className={cn("w-full flex items-center justify-between py-1 px-0.5", className)}>
      ...
      {(!user ? !isPanelOpen : isPanelOpen) && (
        <button
          onClick={handleActionClick}
          className="px-3 py-1 rounded-full transition-all glass-btn-active text-white shadow-lg border-none cursor-pointer"
          style={{ fontWeight: 'normal', fontSize: '14px' }}
        >
          {user 
            ? (t('auth.backToGame') || 'Return to Game') 
            : (t('auth.login') || 'Login')
          }
        </button>
      )}
