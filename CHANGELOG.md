...
- **版权信息入面板**：从双栏外部居中改为右侧面板内底部浮层（`absolute bottom`），不占流、不挤压按钮；全屏下版权完整可见。
- **灯泡提示轮廓柔和化**：提示虚线从刺眼亮绿粗线（方块端点）改为薄荷绿 `rgba(80,220,130,0.9)` 细线 + `lineCap/lineJoin: round`，与拼图块风格统一。
- **全局 emoji → lucide-react SVG**：🏆→Trophy、🔥→Flame、🥇🥈🥉→Medal（金/银/铜）、⚡🎯→Zap/Target、🧘→Focus（同心圆靶）；删除非必要的 👆🎮；i18n 文案清理角度 ✨、结算 🎉🌟。
- **修复空间推理勋章永不亮起**：`calculateFinalScore` 算出的 `rotationEfficiency` 未写回 `completedStats`，存库与勋章判定读到的永远是初始值 0；完成时归一化写回（0-1 小数），效率 ≥90% 正常亮。
- **修复 SSR Hydration 版本号不一致**：dev server 缓存旧 APP_VERSION 导致 1.5.13/1.5.18 文案不匹配，重启后一致。

## [v1.5.18] - 2026-09-19

### 🧩 面板适配规则修正：面板=画布 1:1 + 视窗驱动缩放（恢复既有适配体系）

- **回滚 1.5.17 错误做法**：面板高度不再延长为可用高度，恢复「面板高度 = 画布高度」（画布 1:1 硬约束）；移除面板内容 transform 缩放特技。
- **恢复既有适配规则**：视窗大小 → 画布大小 → `panelScale`（去 0.85 下限）→ `--panel-scale` 统一驱动面板**按钮高/间距/字号/margin**（adaptive-ui 规范「统一乘该系数」完整落地）。
- **结构基准收敛**：按钮高 40/36 → 30/32、内容间距 16→8、小节 margin 14→8、面板 padding 10→8——内容随视窗线性缩放，768 全屏下按钮完整显示在面板内（实测 607 ≤ 602+容差）。
- **极端矮视口（iPad Safari 浏览器 ~650）**：内容缩放至触控下限（按钮 ~22px），按钮滚动可达（面板 overflow 滚动区）；真机全屏 768 与常规浏览器视口完整显示。
- **回归**：Playwright 5 端全绿（21.9s）——新增 `ipad-browser`（1024×650 浏览器视窗）case：按钮滚动可达 + 视口内；`ipad-landscape` 断言按钮 ≤ 面板底边；desktop/手机核心链无回归。

---

## [v1.5.17] - 2026-09-19

### 🧩 iPad 横屏面板溢出修复：内容适配面板 (Panel Fit)

- **根因**：iPad 横屏（1024x768）画布宽受限 → 面板高仅 ~602px，而面板内容（10 种切割类型 + 全部控件）约 739px，底部重玩/重开按钮溢出不可见；且 `.desktop-layout` 整层 touch-action 锁定导致触摸无法滚动面板。
- **面板高度独立**：`panelHeight` 从「= 画布高度」改为「= 可用高度」，面板撑满视口容纳全部控件（桌面同步受益，内容不再超高）。
- **空间驱动紧凑**：`panelScale < 0.9`（iPad 横屏/小窗口）时按钮高度 40→32 / 36→28、内容间距 space-y-4→space-y-2、小节 margin 收紧——字号跟随 panelScale 0.85，底部按钮完整显示。
- **滚动兜底**：`.glass-panel` 放行 pan-y（specificity 高于整层锁定），内容仍超高时可垂直滚动；布局整层 touch-action none 保持（1.5.14 面板拖动修复不回退）。
- **回归**：Playwright 4 端全绿（18.3s）——新增断言「重玩/重开按钮底部 ≤ 面板底边」；desktop/iPad 竖屏/手机核心链无回归；bu 桌面视觉确认面板控件完整。
...
- **CI 移动视口冒烟（Playwright）**：新增 smoke-mobile job（push 后自动运行），iPhone 触控视口跑完整核心链（选形状 → 难度 8 → 嵌齿切割 → 散开 → 拖拽），断言切割完成（散开按钮点亮）、无阻断性错误（hydration/崩溃类）；本地实测 7.2s 通过。
- **封板硬门槛固化（项目 skill）**：三端验证清单升级为强制门槛——涉及布局/渲染/切割/交互的版本必须真实手机实测通过才能封板（1.5.6-1.5.13 教训：桌面通过 ≠ 移动端可交付）；adaptive-ui.md 新增移动端专属问题速查表（1.5.14 修复清单，不得回退）。
- **适配收敛（零风险项）**：touch-action 去除内联冗余，统一由 `.desktop-layout` CSS 类（!important）单一权威控制——消除双写，行为不变。
- **设计原则**：冒烟只防真实回归、断言用户可见行为，不追求覆盖率（吸取早期自嗨式测试教训，避免脚本骗自己）。

---

## [v1.5.14] - 2026-09-19

### ⚡ 移动端切割速度回归修复：10 种切割全量秒切 (Mobile Cut Performance)

- **切割 Worker 抢占 + 忙闲复用**：新切割请求立即中断旧切割（连点/切换不被排队阻塞）；空闲 Worker 复用不重建（消除手机端每次重建数秒冷启）；页面加载预创建 Worker（首次切割免编译等待）；超时 8s→20s（防手机冷启误超时降级同步卡死）。
- **放射切割顺序分割**：利用"单点发射、互不交叉、到达边界"结构直接构扇区（跳过平面图求交），难度 8 从 ~69ms 降至 ~9.5ms（seed 平均），平面图路径保留作回退。
- **图网络网格索引**：computeArrangement 空间索引（CELL=48px），求交从全对（900 万段对）降至共享网格段对（~16ms）。
- **离屏纹理创建治理**：纹理 DPR cap 1 + 物理尺寸 cap 256px（装饰性气孔纹理降采样，drawImage 缩放回逻辑尺寸）——大/复杂碎片（放射扇区、嵌齿锯齿）纹理创建成本与碎裂同档。
- **iPad 横屏面板拖动修复**：desktop 布局整层 touch-action: none，消除 iOS Safari 橡皮筋弹动导致右侧面板被拖动；难度滑条专业体验（8 点位、吸附、逐点音效、端点对齐）。
- **验证**：tsc 0 错误、lint 无告警、几何回归（10 类型 × 8 档 × 2 形状 × 2 seed）全绿；手机实测 10 种切割全部"数 1 下"内完成，拖拽流畅无闪烁。

---

## [v1.5.13] - 2026-09-18

### 🖥️ 移动端渲染优化：DPR 上限 + rAF 批量渲染 (DPR-Capped Retina Rendering)

- **Retina 锐利渲染**：canvas 物理分辨率 = 逻辑 × 设备像素比（DPR 上限 3，防高 DPR 内存爆炸）——2x/3x 屏幕（Mac Retina / iPhone / iPad）拼图边缘与纹理不再模糊，实测 ratioX 1.0 → 2.0（像素量 ×4）。
- **rAF 批量渲染**：同帧内多次状态更新合并为一次绘制（scheduleRender/performRender），高难度 55 片 × 4 倍像素下散开动画仍保持 60fps 满帧、0 掉帧。
- **纹理位图 ×DPR**：离屏缓存按设备像素比生成并随 DPR 变化失效重建，杜绝位图放大模糊；绘制统一走逻辑尺寸，物理像素只作用于画布内部。
- **始终中心原则**：修复切割/散开几何计算误用物理画布尺寸（`canvasRef.current.width`）导致拼图向右下偏移的问题——逻辑坐标系（canvasSize）为唯一权威，拼图始终以画布中心为锚；交互坐标映射同步归一（DPR 后逻辑坐标 = CSS 坐标，命中零错位）。
- **验证**：`tsc` 0 错误、`next lint` 无告警、`next build` 全绿、双切割 verify 脚本全绿；浏览器实测（DPR 2，难度 8 蜂巢 55 片）散开动画 60fps / 0 掉帧、拖拽命中正确、拼图居中无位移。
...
- **增量顺序切割架构与 10 种切割方式 (Incremental Cutting Architecture & 10 Cut Types)**:
  - **增量顺序切割器**（IncrementalCutter）：由"先生成全部切割线再统一求交"改为**逐刀增量切割**——每刀只对当前最大碎片求交、恰好 2 交点后一分为二；交点共享、碎片级面积守恒、缝合缺口恒 0，从根上杜绝曲线交叉飞边。
  - **新增 4 种切割方式**：嵌齿（正弦齿+入口/出口 taper）、S弯（双弧贝塞尔、双曲清晰）、折线（折角 30°~90°、每刀独立折角）、蜂巢（六边形网格）。
  - **贯穿曲线**：真正随机贯穿二次贝塞尔曲线；原"曲线"更名"放射曲线"（星形放射式），显示文案精简为「放射」「曲线」；「凹凸」更名「鱼鳞」/ Fish Scale（枚举值不变，存档兼容）。
  - **全类型统一秒切**：S弯/折线不再逐刀动画，生成完成即一次显示最终碎片。
  - **蜂巢**：所有难度可选；切割后极小碎片（<25% 单元面积）并入相邻块（面积守恒）；跨形状边界溢出碎片过滤；采样自适应由平均 912ms 降至 ≤2ms。
- **面板交互与操作次序优化 (Panel Interaction & Flow Optimization)**:
  - **操作次序**：选形状 → 难度、切割类型按钮点亮（切割按钮暗）→ 选切割类型等同点击切割按钮（自动切割，按钮随即变"再次切割"可重复、散开按钮点亮）→ 选/调难度后，散开前可随时换切割类型重切或重复"再次切割" → 散开后点亮游戏控制；重玩本局/重开游戏全程可用。
  - 切割类型不依赖难度；切割按钮需形状 + 切割类型已选。
  - tab 改名（切割→难度、类型→切割）；移动端隐藏切割类型小标题。
- **稳定性与几何修复 (Stability & Geometry Fixes)**:
  - 曲线/S弯镂空修复：切割交点与路径采样点标记 `isOriginal: false`，渲染精确走 lineTo，共享切割边不再各自贝塞尔化产生缝隙。
  - 直线/斜线低档位片数补偿（有效线判定阈值修正，n 线至少产 n+1 片）。
  - 贯穿曲线难度映射：档位→目标刀数按 `DIFFICULTY_SETTINGS` 映射（8 档 16 片命中"16-30+"下限）。
- **工程规范与全链路扩展验证 (Engineering Standards & Extension Verification)**:
  - 项目专属 Skill [generative-puzzle-extension]：新增切割方式/形状全链路规范（六大生成器分族、十种切割类型清单、新增标准流程、几何硬约束、更名规范、难度档位表、验证基线）+ `verify_extension.ts` 自动验证（i18n 双块、档位片数、废弃残留），10 类型 × 8 档全绿。
...
本版本集中修复 iPhone/iPad 通过 PWA（添加到主屏幕）启动时的视口、安全区及登录交互问题，确保首次打开、旋转屏幕和登录后均能保持完整可用的游戏界面。

- **PWA 首屏全屏显示修复**:
  - 统一使用 `viewport-fit=cover`、动态/大视口高度单位及主题底色，消除首次启动时底部黑边或安全区露底问题。
  - 增加 PWA 独立运行模式识别与视口重排同步，修复首次打开竖屏显示底色、旋转后布局状态不一致的问题。
  - 游戏界面加载完成前保持加载层覆盖，避免启动时先显示背景图、再闪现游戏界面的视觉抖动。
- **PWA 登录弹窗与登录后交互修复**:
  - 移除登录及榜单操作中的延迟 viewport 元数据切换，避免灵动岛区域露出底色以及登录后页面暂时无法点击。
  - 登录成功后立即关闭弹窗并恢复游戏交互，离线成绩同步改为后台执行，不再阻塞界面操作。
  - 弹窗遮罩同步覆盖完整可视视口，改善灵动岛及安全区附近的背景显示。
- **Chrome、Arc 浏览器竖屏改善**:
  - 非 PWA Chrome 使用 `visualViewport` 同步根容器高度，竖屏布局按可视窗口安全居中，改善界面整体向上溢出问题。
- **已知事项**:
  - Chrome、Arc 浏览器横屏时，两侧显示游戏背景色，不影响游戏操作，后续版本继续优化。
  - Safari、微信浏览器竖横屏显示完美适配，后续版本不再优化。
...
本版本专注于优化顶栏全局用户状态按钮在不同登录状态下的视觉呈现与交互，提升多端及不同浏览器下的适配一致性。

- **访客（未登录）状态中性化**:
  - 移除了未登录状态下点击用户按钮时的橙色/金色激活背景，使其与语言切换等其他工具按钮保持一致的玻璃质感。
  - 使用半透明灰/白色图标及边框（`rgba(255,255,255,0.45)`），即使在未登录时点击唤起登录界面，其外观外观和状态也保持中性，不会产生过度引导或色值混淆。
- **已登录状态视觉对齐**:
  - 已登录状态的用户按钮保持与顶栏其他工具按钮（如排行榜、音效、全屏等）一致的金色/琥珀色（`var(--brand-amber)`）图标与边框风格。
  - 移除了已登录状态下激活时大面积高亮金色背景导致图标不可读的问题，仅保留与其他按钮相同的金色/琥珀色高亮边框和高对比度的描边。
  - 移除该按钮内联样式中的硬编码 `backgroundColor` 与 `color` 属性，将其完全抽象到 `globals.css` 类名中进行多端、多浏览器无缝适配。
...
本版本专注于重塑游戏的用户登录注册流程，将复杂的账密输入验证转化为“即输即玩”的极简体验，同时在桌面端实现了与移动端完全对齐的用户入口及无障碍标准优化。

- **极简一体化身份验证 (Unified Enter Game Flow)**:
  - **一键入口**: 取消了原本独立的“注册”与“登录”双按钮，合并为单一的“进入游戏”按钮。
  - **智能鉴权**: 彻底隐藏了注册与登录的路径差异。后台根据输入的昵称与 PIN 自动校验：若匹配现有记录则立即登录并同步成绩，若不存在记录则自动静默注册并开始同步。
  - **文案与提示优化**: 将昵称输入框占位符明确为“昵称（2-8 字符）”，并在底部统一更新了“记住昵称和密码,随时随地看到你的历史成绩和全球排名”的说明。此项修改已在桌面横屏、移动竖屏、移动横屏所有断点全局生效。
- **桌面端登录入口与体验对齐移动端 (Desktop Layout Auth Alignment)**:
  - **入口迁移**: 移除了原先首页突兀的初始登录按钮，将用户入口统一收口至桌面右上角的全局快捷功能区（用户图标），达到全端入口统一。
  - **沉浸式登录交互**: 用户登录面板直接展示在主界面的侧边控制栏。未登录时点击奖杯（榜单）也会唤出登录提示，登录完成后保持在游戏主界面，消除了原有跳转导致的体验割裂。
  - **UI 细节统一**: 为已登录用户新增了多端统一样式的退出确认弹窗，并修复其面板圆角为 `1.5rem` (24px)，确保与桌面主游戏界面的画板圆角形成完美视觉呼应。
- **无障碍访问 (A11y) 与控制台警告修复**:
  - **屏幕阅读器合规**: 针对退出登录的 `AlertDialog` 弹窗组件，注入了屏幕阅读器专用（`sr-only`）的 `AlertDialogTitle` 和 `AlertDialogDescription`，解决了 UI 库控制台关于缺少弹窗描述的报错。
  - **焦点冲突修复**: 在全局用户图标触发弹窗的点击事件中附带了主动失焦 (`blur()`) 逻辑，避免弹窗遮罩展开导致的焦点越权（`aria-hidden` 控制台红字警告）问题。
...
本版本专注于修复移动端主流浏览器（微信、Chrome）在竖屏模式下查看详细成绩时，结算弹窗的排版适配与遮挡溢出问题。

- **微信端顶部留空优化**: 移除了多余的垂直间距，将顶部奖杯图标尺寸微调（`w-14` -> `w-12`）并压缩标题及边距，解决微信浏览器环境内结算弹窗顶部留白过大的视觉瑕疵。
- **Chrome 底部按钮溢出修复**:
    - **视口自适应机制**: 将弹窗卡片的 `max-h-[85vh]` 调整为 `max-h-full`，使其高度约束由外层容器的可用空间动态决定。
    - **安全区补丁**: 为外层覆盖层（Overlay）注入 `safe-area-inset-top` 与 `safe-area-inset-bottom` 动态 padding，精准预留移动端浏览器动态底栏和系统安全区的物理空间，彻底防止底部“重玩”与“新游戏”按钮被遮挡或溢出。
    - **视觉定位纠正**: 保持弹窗在屏幕中的绝对居中（`items-center`），完美兼顾屏幕垂直利用率与人机交互便利性。

---

## [v1.4.20] - 2026-05-15

### 🎵 交互音效增强与结算面板 UI 规范统一 (Audio Polish & Settlement UI Consistency)

本版本对结算面板的小标题样式进行了全面规范统一，并新增了形状拖拽松开后的复原音效，提升了游戏的听觉闭环体验。

- **结算面板小标题统一**: 对游戏结算面板（`DesktopScoreLayout` / `MobileScoreLayout`）内所有分区小标题的字号、字重、颜色及间距进行了全量对齐，消除了多处视觉不一致问题，达成了桌面端与移动端结算 UI 语言的绝对统一。
- **拖拽松开复原音效**: 新增形状拖拽松开后触发的复原音效。当玩家松手后拼图碎片未能吸附到目标位置而回弹时，将播放一段清脆的复原提示音，与已有的吸附"咔嗒"音效共同构建完整的触觉-听觉反馈闭环。

---

## [v1.4.19] - 2026-05-02

### 🌐 国际化与 UI 细节深度优化 (I18n & UI Polish)

本版本专注于修复国际化显示遗留问题，并对难度选择面板进行了精细化调整。

- **国际化修复**: 彻底解决了英文模式下结算面板出现“用时”中英混排的问题，新增 `timeSpent` 翻译键并应用。
- **难度选择面板优化**:
    - **字号调整**: 缩小了难度名称的字号，使其更加精致且平衡。
    - **单位补充**: 在拼图块数范围后新增了 `片 / PCS` 单位显示，增强了信息的可读性。
    - **排版对齐**: 将难度描述文字从居中改为左对齐，优化了横向布局的结构感。

---

## [v1.4.18] - 2026-05-02

### 📊 结算面板成绩明细增强 (Enhanced Score Breakdown)

本版本对结算面板的成绩明细进行了深度优化，为玩家提供更直观的反馈。

- **速度奖励明细优化**: 在结算面板的“速度”项中，新增显示本局实际耗时及对应的时间奖励级别（如：极速、快速、良好等），让玩家清晰了解自己的表现水平。
- **布局一致性**: 统一了移动端（MobileScoreLayout）与桌面端（DesktopScoreLayout）的速度明细显示逻辑，确保全平台体验一致。
