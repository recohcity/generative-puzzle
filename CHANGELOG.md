...
## [v1.3.14] - 2025-06-22
### 主题：构建全自动"测试-分析-洞察"闭环工作流

- 本次更新是一次里程碑式的发布，我们并行完成了两大核心任务，最终将一个脆弱、手动的测试流程，升级为一个**稳定、高效、可视化的全自动性能分析平台**。
- **相关文档**：本次更新的完整实现思路与技术细节，已沉淀为[《全自动"测试-分析-洞察"闭环工作流实战指南》](./docs/automated_testing_workflow.md)。

### 1. 攻坚E2E测试：通过专用测试API实现100%的稳定性
- **背景**: `e2e/full_game_flow.spec.ts` 曾因依赖脆弱的UI模拟而极不稳定。
- **解决方案**: 我们设计并实现了一套**专用于测试的程序化接口（Test API）**，将所有核心交互封装为稳定的函数调用。
- **重构**: 我们基于此API重构了测试脚本，彻底抛弃了 `page.click()` 和 `page.dragAndDrop()` 等不可靠操作。
- **成果**: 获得了一个**100%稳定可靠的自动化测试流程**，并提炼出宝贵经验：**对于复杂应用，构建专用测试API是保障测试质量的根本**。

### 2. 动态可视化：打造免维护的性能趋势分析平台
- **背景**: 以往的性能报告是静态的，需要手动更新数据和重启服务。
- **解决方案**: 我们创建了一个新的Next.js API路由 (`/api/performance-trend/route.ts`)，它会**实时扫描并解析**所有测试报告。
- **前端实现**: 我们构建了一个全新的可视化交互页面 (`/test`)，它会通过fetch上述API来动态加载数据。
- **成果**: 实现了**测试数据的全自动、实时可视化**。现在，每次运行测试后，只需刷新页面即可看到最新的性能趋势图表和详细报告，无需任何手动干预。
